...
The schemas describe **stored records**; creation manifests (e.g. `examples/agent.json`) may omit system-assigned fields (`created_at`, `revision`), which the control plane assigns at registration.
...
## Consequences
The independent CI validator checks representative stored records against every relevant
schema and promotes the checked-in creation examples to stored-record shape before validating
them. Creation manifests have a deliberately narrower runtime validator because their
system-owned fields do not exist yet. The event type registry lives in
`docs/specs/EVENT_TYPES.md` and is updated in the same change as any new event type.
